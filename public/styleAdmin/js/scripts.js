...
!function ($) {
	...
	$.jasmineNoty = function(options){
		...
		focusElement = function(pos){
			jasmine.bodyHtml.animate({scrollTop: pos}, 300, function(){
				el.addClass('in');
			});
		},
		init = function(){
...
!function ($) {
	...
	updateSize = function(){
		...
		var sw = jasmine.container.width(), currentScreen;
...
!function ($) {
	...
	jasmine.window.on('load', function(){
		if (jasmine.mainNav.length) {
			jasmine.mainNav.jasmineAffix({className : 'mainnav-fixed'});
		}

		if (jasmine.aside.length) {
			jasmine.aside.jasmineAffix({className : 'aside-fixed'});
		}
	});
